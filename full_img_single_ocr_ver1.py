"""
Akış Şeması
GRUP 1- Hazırlık ve Yardımcı Fonksiyonlar: Yatırım Uygulaması, açıkmı, maximize mı, arka planda mı vs tüm kontrolleri yapan uyaran veya açıksa öne getiren
 adımlar.
GRUP 2- Tümleşik Resim Oluşturma: Mouse göstergesini ilk sayfada doğru noktaya kaydırıp veri bölgesinden resim alan ardından kaydırma yaparak
 tekrar resim alan, üst üste iki kez aynı resim gelmişse son sayfada olduğunu anlayıp döngüden çıkan bu işlemler sırasında da
   toplanan tüm resimleri birleştirip tek bir resim yapan adımlar
GRUP 3- Satır İşleme: Tek resmi uygun satır yüksekliklerine bölerek tüm satırları okuyan ve her satırın içindeki senet ismi ve
 fiyat verilerini daha yüksek ocr başarımı için ayrıştırıp daha sonra ocr işlemini uygulayan, ardından listede yer alan tekrarlı
   senet isimlerini fiyat en sonuncudan gelecek şekilde teke indirgeyen adımlar.
...
# ==== MAIN PROCESS - BUNDA HERSAYFA TEK TEK OCR VAR. DUZENLENECEK ====
def process_rows():
    global bbox
    ...
    if not bring_investing_app_to_front():
        print("⚠️ Yatırım Platformu Uygulaması açık değil.")
        sys.exit(1)
    ...
    while same_image_flag == False:  #  same_image_flag True olduğunda döngü sonlanacak
        ...
        # Görüntüyü dosyaya kaydediyoruz
        ...
        print(f"📸 Alınan resimler kaydedildi: {filename}")
        ...
        if hash_previous_img is None:
            ...
            if diff == 0:  # Fark yok
                ...
                print("🟡Aynı resim geldi. Döngü sonlandırılacak.")
            else:
                same_image_flag = False        
                print('Yeni resim geldi.Devam ediliyor...')

        # Çekim sonrası kontrol
        if full_img is None:
            print("  • Çekim başarısız!")
        else:
            print(f"  • Çekim başarılı - Görüntü boyutu: {full_img.shape}")
        ...
        if same_image_flag:
            ...
            #merged_img.show()
            merged_img.save("merged_output.png")
            ### İPTAL EDİLDİ all_results = remove_duplicates(all_results)
        else:
            # Mevcut sonuçları önceki sonuç olarak kaydet
            ### İPTAL EDİLDİ previous_results = results.copy()
        ...
        scroll_down()
        time.sleep(0.4)  # Kararlı görüntü bekleme

    scroll_to_top_fast()
    print(f"\n✅ İşlem tamamlandı! Toplam {page_number-1} sayfa tarandı.")
    ### Aşağıdaki komut ocr tarafında da olacak
...
def process_image(image_path):
    img, thresh = load_and_preprocess_image_original(image_path)
    img, equalized = load_and_preprocess_image_actual(image_path)
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print('merged_output.png üretim süresi', end_time - start_time)